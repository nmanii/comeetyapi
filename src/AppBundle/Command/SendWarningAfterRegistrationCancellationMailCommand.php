...
class SendWarningAfterRegistrationCancellationMailCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('mail:warning-after-cancellation-mail')
            ->setDescription('Send a mail to all user who cancelled their participation')
            ->setHelp("Send a mail to all user who cancelled their participation");
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($usersFeedbacks as $usersFeedback) {
            $mailLog = $mailLogRepository->findOneBy(['user'=>$usersFeedback->getReference(), 'mailName'=>'warningAfterRegistrationCancellation']);
            if(empty($mailLog)) {
                $reportedUser = $userRepository->findOneById($usersFeedback->getReference());
                if(!empty($reportedUser)) {
                    $mailManager->SendWarningAfterRegistrationCancellation();
                }
                $count++;
            }
        }
